...
parser.add_argument("--model_name", type=str, default="SimpleCNN", help="模型名称，例如 SimpleCNN")
...
parser.add_argument("--train_dir", type=str, default=r"AI02-CNN/data/train", help="训练集路径")
parser.add_argument("--test_dir", type=str, default=r"AI02-CNN/data/test", help="验证集路径")
...
train_dataset = CatDogDataset(args.train_dir, transform=train_transform)
# 创建验证集对象，加载路径data2\test下的图像，应用验证时的固定预处理（如中心裁剪），保证验证结果稳定
test_dataset = CatDogDataset(args.test_dir, transform=train_transform)

# 创建数据加载器
# DataLoader就是用来把你的数据集“打包好，一批一批地送进模型里”的工具
train_loader = DataLoader(
    train_dataset,       # 传入训练数据集对象
    batch_size=args.batch_size,       # 每个批次加载32张图像
...
test_loader = DataLoader(
    ...
    batch_size=args.batch_size,       # 验证时也用相同批大小
...
train_model(model, train_loader, test_loader, loss_fun, optimizer, args.epochs)
